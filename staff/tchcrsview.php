...
try{
	...
	$sql="SELECT * FROM lecturer where phone_number=? and level=? GROUP BY course";
	$query=$dbh->prepare($sql);
	$phone=$_GET['phone'];
	$level=$_GET['level'];
	$query->execute(array($phone,$level));
...
<div class="logos">
	...
	<div class="backhome">
	<a href="staffdashboard.php"><img src="icons/home6.png" class="homeicon"></a>
	<a href="staffdashboard.php"><div class="linkhome">Home</div></a>
...
<?php
	...
	?>
	<form class="tch_det">	
  <input type="text" placeholder="Lecturer's first name" name="" value="<?php echo $result->lecture_firstname;?> <?php echo $result->lecture_lastname;?>">
</form>
</div>
...
<div class="tchlist">Lecturer Attendance View</div>
...
foreach ($results as $result) {
	?>	
		...
		<input type="text" name="pid" value="<?php echo $result->phone_number;?>" hidden>	
...
if (isset($_POST['result'])) {
	...
	$pid = $_POST['pid'];
	$dbh= new PDO("mysql:host=localhost;dbname=attendance", "root", "");
	$sql="SELECT * FROM lecturer where course='".$cid."'  AND phone_number = '".$pid."' ";
	$query=$dbh->prepare($sql);
	$query->execute();
	$results=$query->fetchAll(PDO::FETCH_OBJ);
...
$pid = $_POST['pid'];
$dbh= new PDO("mysql:host=localhost;dbname=attendance", "root", "");
$sql= "SELECT status FROM lecturer WHERE phone_number = '".$pid."' AND course='".$cid."' AND status='present' ";
$count=$dbh->prepare($sql);
$count->execute();
$cnt=$count->rowCount();

$sql2= "SELECT status FROM lecturer WHERE phone_number='".$pid."' AND course='".$cid."' ";
